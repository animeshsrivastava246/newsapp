...
export class News extends Component {
	...
	constructor(props) {
		...
		document.title = `NewsPaper - ${this.capitalizeFirstLetter(
			this.props.category
		)}`;
	}
	...
	async updateNews() {
		...
		let data = await fetch(
			`https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=${this.props.apiKey}&page=${this.state.page}&pageSize=${this.props.pgSize}`
		);
		...
		return (
			<>
				<h2
					...
					style={{ margin: "30px 0px", marginTop: "56px" }}
				>
					<strong>
						<i>
							NewsPaper - {this.capitalizeFirstLetter(this.props.category)} -
							Top Headlines
						</i>
					</strong>
				</h2>
							...
							{this.state.articles.map((elem) => {
								return (
									<div className="col-md-3" key={elem.url}>
										<NewsItem
											title={elem.title ? elem.title : "Title"}
											description={elem.description ? elem.description : ""}
											imgUrl={
												elem.urlToImage
													? elem.urlToImage
													: "https://wallpaperaccess.com/full/2112629.jpg"
											}
											newsUrl={elem.url}
											author={elem.author}
